...
2. Wait for manual approval (GitHub Environments → production)
         ↓
3. Copy `docker-compose.yml` lên server (scp)
         ↓
4. SSH vào server:
     - Pull new images
...
> **Server KHÔNG cần là git repo.** CI copy thẳng `docker-compose.yml` từ runner
> lên `DEPLOY_PATH` ở mỗi lần deploy — không chạy `git` trên server.

> **Reverse proxy do host tự quản.** Stack KHÔNG còn nginx container. Compose chỉ
> publish `backend-api:8080` và `frontend:3000`; nginx (hoặc Caddy/Traefik) cài sẵn
> trên host trỏ domain vào 2 cổng đó — xem mục 4b.

> **Postgres + Redis KHÔNG nằm trong compose.** Phải cài sẵn trên host (hoặc dịch
> vụ managed). `.env` mặc định trỏ `host.docker.internal` — compose đã map
> `host-gateway` để container Linux resolve được tên này.
...
# B. Tạo thư mục deploy (KHÔNG cần git clone — CI sẽ scp compose vào đây)
sudo mkdir -p /opt/mmo
sudo chown $USER:$USER /opt/mmo
cd /opt/mmo

# B2. Đặt sẵn compose cho lần bring-up đầu tiên (trước deploy CI đầu tiên).
#     Chỉ cần đúng 1 file ./docker-compose.yml (reverse proxy do host tự quản).
curl -fsSL -o docker-compose.yml \
  https://raw.githubusercontent.com/ngthotuan/mmo/main/docker-compose.yml
# (Nếu repo private: scp file này từ máy local lên, hoặc git clone 1 lần.)
...
## 4b. Reverse proxy trên host (nginx có sẵn)

Stack không còn nginx container. nginx host của bạn trỏ domain vào 2 cổng đã publish.
Khuyến nghị serve **cùng 1 domain** (same-origin → không cần CORS):

```nginx
server {
    listen 80;                       # + 443 với cert (Let's Encrypt / certbot)
    server_name your-domain.com;

    client_max_body_size 500M;       # ⚠️ upload video — thiếu sẽ 413
    ...
    location /api/ {
        ...
        proxy_read_timeout 300s;     # ⚠️ pipeline AI/render chạy lâu
    }
...
Vì serve same-origin, đặt `NEXT_PUBLIC_API_URL=https://your-domain.com` (origin, KHÔNG
có `/api`) — FE tự thêm `/api/v1`, nginx route `/api/` về backend.
...
```env
APP_ENV=production
FRONTEND_URL=https://your-domain.com        # dùng cho CORS + OAuth redirect
NEXT_PUBLIC_API_URL=https://your-domain.com # origin, KHÔNG /api (build-time, xem ghi chú)
...
> **Lưu ý `NEXT_PUBLIC_API_URL` (build-time, KHÔNG phải runtime):** biến này được
> inline vào bundle frontend lúc **build image** — sửa `.env` trên server KHÔNG có
> tác dụng với frontend. `release.yml` truyền nó vào build-arg từ **repo variable**
> `NEXT_PUBLIC_API_URL`, nên bắt buộc:
>
> **Settings → Secrets and variables → Actions → Variables → New variable**
> `NEXT_PUBLIC_API_URL = https://your-domain.com` (origin, KHÔNG có `/api`).
>
> Nếu để trống, image sẽ fallback `http://localhost:8080` (xem `client.ts`) → trình
> duyệt người dùng prod gọi vào localhost của chính họ → mọi request fail. Đổi domain
> ⇒ phải push tag mới để build lại image (không sửa runtime được).
...
| `fatal: not a git repository` ở bước deploy | Phiên bản workflow cũ chạy `git` trên server. Đã fix: CI scp compose, không dùng git. Đảm bảo `release.yml` đã cập nhật. |
...
| Container không kết nối được DB/Redis (`host.docker.internal` không resolve) | Trên Linux cần `extra_hosts: host-gateway` (đã có trong compose). Hoặc DB/Redis chưa chạy / chưa mở cho container truy cập. |
| Browser prod gọi `http://localhost:8080` → mọi API fail | Chưa set repo variable `NEXT_PUBLIC_API_URL` lúc build. Xem ghi chú mục 5, push tag lại để build image. |
| Upload video bị `413 Request Entity Too Large` | Thiếu `client_max_body_size 500M` trong nginx host (mục 4b). |
